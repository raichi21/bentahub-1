...
- [🏗️ Drizzle Schema Definition Patterns](#️-drizzle-schema-definition-patterns)
- [🧩 Feature Component Patterns](#-feature-component-patterns)
- [🔐 Permission System Pattern](#-permission-system-pattern)
- [🛡️ Middleware Pattern](#️-middleware-pattern)
- [⚙️ Task Script Pattern](#️-task-script-pattern)
...
- **Shared to Feature**: Features CAN import from global shared folders (e.g., `src/components/`, `src/lib/`, `src/server/`).
...
- **Shared Imports Restrictions**: Shared folders are not allowed to import items from features or app folders.
...
> - `.eslintrc.json` (under `boundaries/element-types` rules)
> - `independentModules.jsonc` (project-structure rules)
...
- **Public Cached Wrapper**: Exported from the `db` file. Wraps the internal query using the `dbCache` wrapper (Next.js `unstable_cache` helper) defined in `cache.ts`.
- **Private Uncached Internal Query**: Prefixed with `*Internal`. Contains the actual Drizzle ORM query logic. It is not exported.
...
Three standardized cache tagging functions are defined in `cache.ts`:
...
Like all other layers in the codebase (UI components, hooks, validation schemas, and types), database queries and mutations follow the general **Rule of Promotion**. To maintain strict horizontal boundaries, features are strictly forbidden from importing any code directly from other features.
...
- **Named HTTP Method Exports**: Export named async functions matching HTTP verbs (`GET`, `POST`). Default exports are forbidden.
- **Response Construction**: Return standard `Response` objects (e.g., `new Response("body", { status, headers })`) rather than using helper structures like `NextResponse.json()`.
- **Execution Runtime**: For endpoints requiring fast regional/geographic data (like geolocation lookups), specify the edge runtime: `export const runtime = "edge"`.
- **CORS Headers**: Set CORS headers manually when public access is required.
- **Payload Security & Verification**:
...
- **No Server Action Calls**: API routes must perform database operations by directly importing functions from features' `db` folders (`src/features/*/server/db/*`). They must never invoke server actions.
...
- **File Naming**: Reside in `src/features/[feature-name]/schemas/` under filenames corresponding to features (e.g., `products.ts`).
- **Schema Naming**: Named exports in camelCase with a `Schema` suffix (e.g., `productDetailsSchema`).
- **Form-Oriented Definition**: Zod schemas are written from the perspective of form field inputs, rather than generated automatically from database schemas (e.g., no `createInsertSchema` from drizzle-zod).
- **Input Constraints**:
...
- **Cross-Field Refinement**: Use `.refine()` to validate fields that depend on one another. Always specify `path: ["root"]` to attach refinement errors to the form container.
...
- **File-Level Directive**: Place `"use server"` on the very first line of the file.
...
- **Input Parameter Naming**: Name input parameters `unsafeData` to indicate that input is untrusted. Immediately call `schema.safeParse(unsafeData)` to validate on the server side.
- **Error Handling & Return Contract**:
...
- **Data Layer Delegation**: Actions must import database mutations and queries under aliased names (e.g. `createProduct as createProductDb`) and delegate database writes to them.
...
All database table, relationship, index, and enum schemas are defined in `schema.ts`.

### Key Rules

- **Single Schema File**: All tables are declared in `schema.ts`.
- **Naming Conventions**:
...
- **Primary Keys**: Always use UUID primary keys: `uuid("id").primaryKey().defaultRandom()`.
- **Shared Timestamps**: Define `createdAt` and `updatedAt` once in the module scope and reuse them across tables. `updatedAt` uses `.$onUpdate(() => new Date())` for automatic timestamp updates.
- **Foreign Keys**: Define cascading deletes explicitly on referential columns: `.references(() => Table.id, { onDelete: "cascade" })`.
- **Unique Constraints**: For 1-to-1 relations, mark the reference column as `.unique()`.
- **Indexes**: Declare indices inside the third argument of `pgTable` (e.g. `index("table.column_index").on(table.column)`).
- **ORM Relationships**: Declare relationships separately using the `relations` utility from `drizzle-orm`, appending `Relations` to the TypeScript export.
...
- **Interactive Client Components**: Mark client files executing form actions or hook logic with `"use client"`.
- **Form Architecture**:
...
- **Non-Form Interactions**: For regular element buttons triggering deletion actions, wrap mutations inside React's `useTransition` to track pending state.
- **Data Visualization**: Recharts components are placed inside features and wrapped in a shadcn `ChartContainer`.
...
- **Exports**: Use named exports for all React components.
...
- **Centralized Evaluation Functions**: Reside in `permissions.ts`.
- **Access Contract**: Evaluate parameters asynchronously returning a `Promise<boolean>` output: `async (userId: string) => Promise<boolean>`.
- **Database Checks**: Query subscription limits or record counts directly from feature database services (e.g., retrieving `getProductCount` from products and checking constraints against tier definitions).
- **Conditional UI Gating (Server Side)**: Protect parts of pages by wrapping them in the async `HasPermission.tsx` server component, providing optional fallback elements when permissions check fails.
...
Request authentication routing rules are parsed globally inside `middleware.ts`.

### Key Rules

- **Public/Protected Split**: Protect routes using Clerk's `clerkMiddleware` and `createRouteMatcher`. Match routes matching landing, authentication, and webhook urls.
- **Bypass Rules**: API paths (`/api(.*)`) are marked public at the middleware level; webhook authorization is evaluated individually inside API handlers using cryptographic sign verifications.
...
- **Codebase Isolation**: Task files are reserved exclusively for terminal execution; they must never be imported by application code files.
- **Execution Runner**: Executed via the command-line executor `tsx` directly (e.g. `tsx src/tasks/updateCountryGroups.ts`).
- **No Database Cache Wrapping**: Tasks bypass database wrapper cache layers, retrieving data and updating schemas directly.
- **Conflict Resolution**: Enforce upserts using `onConflictDoUpdate` to cleanly refresh static datasets.
...
- **Providers Wrapper**: The root layout initializes Clerk and shared UI components (e.g. `<Toaster />`).
- **Route Group Layout Styling**:
