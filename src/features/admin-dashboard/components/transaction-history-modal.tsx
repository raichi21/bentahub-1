        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-2">
            ...
            <div className="flex flex-col gap-1 border-t border-border py-4 pr-2">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 mt-0.5 rounded-full text-[10px] font-bold uppercase w-fit ${
            ...
            <div className="flex flex-col gap-1 border-t border-border py-4 pl-2">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 mt-0.5 rounded text-[10px] font-bold uppercase w-fit ${
                transaction.paymentMethod === "cash" ? "bg-muted text-muted-foreground" : "bg-accent text-primary"
              }`}>
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <thead className="bg-muted/10 border-b border-border">
                    <tr className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                      ...
                      <th className="px-4 py-3 text-right w-[17%]">Price</th>
                      <th className="px-4 py-3 text-right w-[18%]">Subtotal</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/30 text-foreground">
                    {transaction.items.map((item, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="px-4 py-3 text-right font-medium">{formatPrice(item.subtotal)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-2">
            <div className="flex justify-between py-1.5">
              ...
              <p className="text-sm text-foreground">{transaction.subtotalDisplay}</p>
            </div>
